...
class HomeUKSpider(scrapy.Spider):
    ...
    extract_url = 'https://www.home.co.uk/for_rent/{post_code}/current_rents?location={post_code}'
    ...
    def extract_result(self, response):
        ...
        by_number_tr = response.xpath('//h2[contains(., "by Number of Bedrooms")]/following-sibling::div[1]/table/tr')
        ...
        for i, tr in enumerate(by_number_tr):
            ...
            if i == 0:
                ...
                name = td[0].xpath('.//text()').extract_first(default='').strip()
                by_number_data[name] = {
                    # '_StartTime': type_start,
                    'properties': td[1].xpath('.//text()').extract_first(default='').strip(),
                    'Average rent': td[2].xpath('.//text()').extract_first(default='').strip(),
                    'Median rent': td[3].xpath('.//text()').extract_first(default='').strip(),
                    'Average ToM': td[4].xpath('.//text()').extract_first(default='').strip(),
                }
